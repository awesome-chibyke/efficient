...
class PaymentGatewayBox extends Model
{
    ...
    public STATIC function getBankDetailsFlutterWave(){
        ...
        $flutterWaveBankCodes = \App\BankCodesModel::where('type_of_gateway', 'flutterwave')->get();
    ...
    public STATIC function getFlutterWaveBankDetailsForDatabase($school_id = 0){
        ...
        $flutter_wave_details = \App\PaymentGatewayBox::where('school_id', $school_id)->where('keyword', 'flutter_wave')->first();
        $flutter_wave_details_array = \App\PaymentGatewayBox::recreateGatewayMangerField($flutter_wave_details);
        ...
        foreach($available_country_code as $k => $each_country_code){
            ...
            if(count($results->data->Banks) > 0){
                ...
                foreach($results->data->Banks as $bank_details){
                    ...
                    $particularBankDetails = \App\BankCodesModel::where('bank_codes', $bank_details->Code)->where('type_of_gateway', 'flutterwave')->first();

                    if($particularBankDetails === null){
                        $bank_codes = new BankCodesModel();
                        $bank_codes->unique_id = Controller::createUniqueId('bank_codes_models', 'unique_id');
